...
$(window).on('load', function () {
	$('.lds-ellipsis').fadeOut(0); // will first fade out the loading animation
	$('.preloader').delay(0).fadeOut(0); // will fade out the white DIV that covers the website.
	$('body').delay(0);
});
...
$('.smooth-scroll,.idocs-navigation a').on('click', function() {
	...
	$('html, body').stop().animate({
      scrollTop: $(sectionTo).offset().top - 120}, 0, 'easeInOutExpo');
});
